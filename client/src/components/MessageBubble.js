...
function getUserColor(name) {
  const colors = [
    ...
    'text-purple-400', 'text-cyan-400', 'text-orange-400', 'text-rose-400',
    'text-emerald-400', 'text-indigo-400', 'text-teal-400', 'text-amber-400',
  ];
  let hash = 0;
...
function getUserColorLight(name) {
  const colors = [
    'text-pink-600', 'text-blue-600', 'text-green-600', 'text-yellow-600',
    'text-purple-600', 'text-cyan-600', 'text-orange-600', 'text-rose-600',
    'text-emerald-600', 'text-indigo-600', 'text-teal-600', 'text-amber-600',
  ];
  let hash = 0;
  for (let i = 0; i < name.length; i++) hash = name.charCodeAt(i) + ((hash << 5) - hash);
  return colors[Math.abs(hash) % colors.length];
...
export default function MessageBubble({ message, isOwn, isLight }) {
  ...
  return (
    <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-1 group`}>
      ...
      >
        {!isOwn && (
          <p className={`text-xs font-semibold mb-0.5 ${isOwn ? 'text-purple-200' : nameColor}`}>
            {message.username}
          </p>
        )}
        ...
        <p className={`text-[10px] mt-0.5 ${isOwn ? 'text-purple-200' : isLight ? 'text-gray-400' : 'text-white/40'} ${isOwn ? 'text-right' : ''}`}>
