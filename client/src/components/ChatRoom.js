...
export default function ChatRoom({ roomId, username, onLeave }) {
  ...
  useEffect(() => {
    ...
    socket.on('user-joined', ({ username: user, users: userList }) => {
      ...
      setMessages((prev) => [
        ...prev,
        { id: `sys-${Date.now()}`, type: 'system', text: `${user} joined the room`, timestamp: Date.now() },
      ]);
    });
    ...
    socket.on('user-left', ({ username: user, users: userList }) => {
      ...
      setMessages((prev) => [
        ...prev,
        { id: `sys-${Date.now()}`, type: 'system', text: `${user} left the room`, timestamp: Date.now() },
      ]);
    });
  ...
  if (error) {
    const isRoomMissing = /room not found/i.test(error);
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-8 max-w-md w-full border border-white/20 text-center">
          ...
          <h2 className="text-xl font-bold text-white mb-2">{isRoomMissing ? 'Room Not Found' : 'Unable to Join Room'}</h2>
    ...
    <div className={`h-screen flex flex-col bg-gradient-to-br ${themeBg}`}>
      ...
      <header className={`flex items-center justify-between px-4 py-3 border-b ${isLight ? 'bg-white/80 border-gray-200' : 'bg-black/20 border-white/10'} backdrop-blur-xl z-10 shrink-0`}>
        <div className="flex items-center gap-3 min-w-0">
          <button
            onClick={onLeave}
            className={`p-2 rounded-lg transition-colors shrink-0 ${isLight ? 'hover:bg-gray-100 text-gray-600' : 'hover:bg-white/10 text-white/70'}`}
            title="Leave Room"
          >
        ...
        <div className="flex items-center gap-1">
          {roomInfo.createdBy === username && (
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${isLight ? 'hover:bg-gray-100 text-gray-600' : 'hover:bg-white/10 text-white/70'}`}
              title="Settings"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className={`px-3 py-3 border-t ${isLight ? 'bg-white/80 border-gray-200' : 'bg-black/20 border-white/10'} backdrop-blur-xl shrink-0`}>
            <form onSubmit={handleSend} className="flex items-end gap-2">
              ...
              <button
                type="submit"
                disabled={!text.trim()}
                className="p-2.5 rounded-xl bg-purple-600 text-white hover:bg-purple-500 transition-colors disabled:opacity-30 disabled:cursor-not-allowed shrink-0"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Modals */}
      {showShare && (
        <ShareModal roomId={roomId} onClose={() => setShowShare(false)} isLight={isLight} />
      )}
      {showSettings && (
        <SettingsModal
          roomInfo={roomInfo}
