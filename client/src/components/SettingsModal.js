    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className={`text-lg font-bold ${isLight ? 'text-gray-900' : 'text-white'}`}>Room Settings</h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          <div>
            <label className={`block text-xs font-medium mb-1.5 uppercase tracking-wider ${isLight ? 'text-gray-500' : 'text-white/50'}`}>
              Room Name
            </label>
            <input
              type="text"
