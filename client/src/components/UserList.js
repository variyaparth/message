...
export default function UserList({ users, currentUser, createdBy, isLight, onClose }) {
  return (
    <div className={`w-64 border-l shrink-0 flex flex-col ${
      isLight ? 'bg-white/80 border-gray-200' : 'bg-black/20 border-white/10'
    } backdrop-blur-xl`}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-inherit">
        ...
        <button
          onClick={onClose}
          className={`p-1 rounded-lg transition-colors ${isLight ? 'hover:bg-gray-100 text-gray-500' : 'hover:bg-white/10 text-white/50'}`}
        >
